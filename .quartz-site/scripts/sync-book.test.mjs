...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.match(
        abilityRow ?? '',
        /^\| Способность эффектов \| 2 \| Краткое описание способности эффектов\. \| Полное описание способности эффектов\. \| 1\/сцену \| Мистика \| Действие \| 30 м \| Все цели в зоне \| Круг, диаметр 15 м \| Стойкость \| До конца сцены \| 300 \|$/u
      );
    } finally {
      if (previousDocsRepoEnv === undefined) {
        delete process.env.PROJECT_ANDROMEDA_DOCS_REPO;
