...
test("buildAbilityCatalogHtml normalizes filter values and exposes the full dropdown dictionaries", () => {
  ...
  assert.ok(skillBlock.includes("\u0425\u0430\u043a\u0435\u0440\u0441\u0442\u0432\u043e"))
...
test("buildRulebookCatalogHtml renders armor defense tags and filters with full defense names", () => {
  ...
  assert.ok(html.includes('data-filter-dropdown="rank"'))
  assert.ok(html.includes('data-filter-dropdown="defense"'))
  assert.ok(html.includes("Бонус к Защите"))
