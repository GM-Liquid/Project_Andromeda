...
type RulebookCatalogFilterField = 'rank' | 'frequency' | 'skill' | 'actions' | 'damage' | 'defense';
...
const catalogHeaderAliases: Record<CatalogHeaderField, string[]> = {
  ...
  credits: ['цена', 'цена:', 'цена в кредитах', 'цена в очках развития'],
...
> = {
  physicalDefense: [
    ...catalogHeaderAliases.physicalDefense,
    '\u0441\u0442\u043e\u0439\u043a\u043e\u0441\u0442\u044c'
  ],
  magicalDefense: [
    ...catalogHeaderAliases.magicalDefense,
    '\u043a\u043e\u043d\u0442\u0440\u043e\u043b\u044c'
  ],
...
const armorSortOptions: RulebookCatalogSortOption[] = [
  ...
  { value: 'price-asc', label: 'По цене', sortKey: 'price', direction: 'asc' },
  {
    value: 'physical-defense-desc',
    label: 'По физической',
    sortKey: 'physicalDefense',
    direction: 'desc'
  },
  {
    value: 'magical-defense-desc',
    label: 'По магической',
    sortKey: 'magicalDefense',
    direction: 'desc'
  },
  {
    value: 'psychic-defense-desc',
    label: 'По психической',
    sortKey: 'psychicDefense',
    direction: 'desc'
  }
];
...
function resolveArtifactCatalogColumns(headers: string[]): ArtifactCatalogColumns | null {
  const descriptionColumns = resolveDescriptionColumns(headers);
  if (!descriptionColumns) {
    return null;
  }

  const columns: ArtifactCatalogColumns = {
    name: findColumn(headers, catalogHeaderAliases.name),
    rank: findColumn(headers, catalogHeaderAliases.rank),
    frequency: findColumn(headers, catalogHeaderAliases.frequency),
    actions: findColumn(headers, catalogHeaderAliases.actions),
...
export function isArtifactCatalogTable(headers: string[]) {
...
function deriveActionLabel(description: string) {
  ...
  if (normalized.startsWith('три действия') || normalized.startsWith('два действия'))
    return 'Маневр';
...
function normalizeDefenseLabel(value: string) {
  ...
  if (
    armorDefenseAliases.physicalDefense.some((alias) => normalized === normalizeCatalogValue(alias))
  ) {
    return armorDefenseLabels.physicalDefense;
  }

  if (
    armorDefenseAliases.magicalDefense.some((alias) => normalized === normalizeCatalogValue(alias))
  ) {
    return armorDefenseLabels.magicalDefense;
  }

  if (
    armorDefenseAliases.psychicDefense.some((alias) => normalized === normalizeCatalogValue(alias))
  ) {
...
function buildArtifactCatalogModel(headers: string[], rows: string[][]): RulebookCatalogModel {
  const columns = resolveArtifactCatalogColumns(headers);
  if (!columns) {
    return { entries: [], filters: [] };
  }
  ...
  return {
    ...
    filters: [
      ...
      },
      {
        ...
        field: 'damage',
        label: 'Урон',
        values: sortNumericValues(
          entries.map((entry) => getSingleFilterValue(entry.filters.damage))
        )
      }
    ]
  };
...
function renderCatalogPrice(entry: RulebookCatalogEntry) {
  if (!entry.price) {
    return '—';
  }

  const unit = entry.priceUnit === undefined ? 'кр' : entry.priceUnit;
  return `${escapeHtml(entry.price)}${unit ? ` ${escapeHtml(unit)}` : ''}`;
}
...
function renderCatalogRangeDropdown(
  filter: Extract<RulebookCatalogFilterDefinition, { kind: 'range' }>
) {
  return `
    <div
      class="${abilityCatalogClass}__filter-group ${abilityCatalogClass}__dropdown-group"
