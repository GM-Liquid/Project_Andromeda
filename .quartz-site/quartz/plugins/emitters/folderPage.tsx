...
function isEmittableFolder(folderName: SimpleSlug): boolean {
  return (
    folderName !== "." && folderName !== "tags" && folderName !== "rulebook"
  );
}
  ...
  return {
    ...
    async *emit(ctx, content, resources) {
      ...
      const folders: Set<SimpleSlug> = new Set(
        allFiles.flatMap((data) => {
          return data.slug
            ? _getFolders(data.slug).filter(isEmittableFolder)
            : [];
        }),
      );
    ...
    async *partialEmit(ctx, content, resources, changeEvents) {
      ...
      for (const changeEvent of changeEvents) {
        ...
        const folders = _getFolders(slug).filter(isEmittableFolder);
