...
export function isRulebookSlug(slug?: string): slug is FullSlug {
  return typeof slug === 'string' && slug.startsWith('rulebook/');
}
...
export function getRulebookHeaderEntries(): RulebookEntry[] {
  return rulebookEntries.filter((entry) => entry.showInHeaderNav);
}
...
export function resolveRulebookAsset(
  slug: FullSlug,
