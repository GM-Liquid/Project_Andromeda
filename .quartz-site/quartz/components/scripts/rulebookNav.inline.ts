...
function closeAllRulebookItems(nav: ParentNode) {
  nav.querySelectorAll(".rulebook-nav-item[data-open='true']").forEach((item) => {
...
function setupRulebookNav() {
  ...
  navs.forEach((nav) => {
    const toggle = nav.querySelector(".rulebook-nav-toggle") as HTMLElement | null
    const setExpanded = (expanded: boolean) => {
      nav.setAttribute("data-expanded", expanded ? "true" : "false")
    ...
    const onToggleClick = () => {
      ...
      setExpanded(nav.getAttribute("data-expanded") !== "true")
    }
    ...
    flyoutItems.forEach((item) => {
      const flyout = item.querySelector(".rulebook-nav-flyout") as HTMLElement | null
      ...
      const onKeyDown = (event: KeyboardEvent) => {
        ...
        closeRulebookItem(item)
        const link = item.querySelector(".rulebook-nav-link") as HTMLElement | null
        link?.focus()
      }
      ...
      window.addCleanup(() => item.removeEventListener("mouseenter", onMouseEnter))
      window.addCleanup(() => item.removeEventListener("mouseleave", onMouseLeave))
  ...
  const onViewportChange = () => {
    if (desktopRulebookNav.matches) {
      document.querySelectorAll(".rulebook-nav").forEach((nav) => {
        nav.setAttribute("data-expanded", "false")
      })
      return
    }

    document.querySelectorAll(".rulebook-nav-item[data-open='true']").forEach((item) => {
      closeRulebookItem(item as HTMLElement)
    })
  }
