...
test('HUD stat grid keeps stress and progression columns compact', () => {
  ...
  assert.match(
    stylesheet,
    /\.project-andromeda \.andromeda-hud--gm \.andromeda-hud__stats-row \{[\s\S]*grid-template-areas:\s*'stress \.'\s*'speed heat';[\s\S]*grid-template-columns:\s*4\.75rem 4\.75rem;/
  );
  assert.match(
    stylesheet,
    /\.project-andromeda\.elite \.andromeda-hud--gm \.andromeda-hud__stats-row \{[\s\S]*grid-template-areas:\s*'stress glory'\s*'speed heat';[\s\S]*grid-template-columns:\s*4\.75rem 8rem;/
  );
  // Opponents pay Heat for their abilities, so the GM HUD tracks it beside Speed.
  assert.match(
    stylesheet,
