...
- Skills are still assigned to the Body / Mind / Spirit categories in data (`CONFIG.ProjectAndromeda.skillCategories`), but those categories are not characteristics, have no independent values, and are **no longer shown as headings on the sheet**: the actor sheet renders skills as a single flat list (`context.skillList`) in category order. The three defenses are likewise **not** attached to skill-category headers anymore; they live in a dedicated "Defenses" block at the top of the skill rail (`context.defenseRows`, base value only — temporary defense modifiers stay in the HUD).
- Skill rank and the character's rank are displayed in the UI as **Roman numerals** (I–V) via `module/helpers/roman.mjs` (`toRoman`), so a rank never reads as the same kind of value as the arabic skill value/bonus. In the sheet's view mode the rank shows as a read-only Roman numeral; in edit mode the numeric input is shown instead (CSS-toggled by `andromeda-play-mode` / `andromeda-edit-mode`). The item "Check" column shows skill name + Roman rank + `+bonus`, without the `2d8` dice (the `2d8` formula is still used for the actual roll, just not displayed).
...
- Dropping an archetype (`_onDropArchetype` in `module/sheets/actor-sheet.mjs`) replaces any existing archetype and the ability it previously granted, creates the compendium-linked archetype, grants the linked signature ability (flagged `flags.project-andromeda.grantedByArchetype`), and raises the archetype skill by the rank bonus (`+1`, capped at the archetype rank cap) so a fresh skill starts at rank 2. Archetypes only apply to `playerCharacter`.
- Removing an archetype reverts what it granted: `clearArchetypeEffects` deletes the granted ability, lowers the archetype skill rank by the same `+1` bonus (floored at rank 1, so any rank the player bought on top is preserved), and resets the three defenses to the rank default (defense = rank). This runs from the `deleteItem` hook on manual deletion (only for the acting user, which then re-renders the sheet because the revert update uses `render: false`) and inline during a drop-replace (the replace delete passes `ARCHETYPE_SWAP_OPTION` so the hook does not double-run). The archetype skill is marked with a star on the sheet via `skill.isArchetypeSkill`.
